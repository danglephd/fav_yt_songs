...
export class MySongsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private writeCachedSongs(songs: Song[]): void {
    localStorage.setItem('cachedSongs', JSON.stringify(songs));
  }

  private updateCachedSong(id: string, updates: Partial<Song>): Promise<void> {
    const songs = this.readCachedSongs();
    const songIndex = songs.findIndex(song => song.id === id);
    if (songIndex >= 0) {
      songs[songIndex] = { ...songs[songIndex], ...updates };
      this.writeCachedSongs(songs);
    }
  ...
  loadSongs(): void {
    this.isLoading = true;
    // load song từ local storage, nếu không có thì hỏi để  load từ database 
    const cachedSongs = localStorage.getItem('cachedSongs');
    if (cachedSongs && cachedSongs !== '[]') {
        this.songService.getActiveSongsFromCache().subscribe(
          (songs: Song[]) => {
            this.songs = songs || [];
      ...
      if (confirm('No cached songs found, do you want to load songs from the database?')) {
        this.songService.getActiveSongs().subscribe(
          (songs: Song[]) => {
            ...
            this.applyFilter();
            this.isLoading = false;
            // save songs to local storage for caching
            localStorage.setItem('cachedSongs', JSON.stringify(this.songs));
          },
          (error) => {
            this.showMessage('Error loading song list', 'error');
            this.isLoading = false;
  ...
  deleteSong(id: string): void {
    if (confirm('Are you sure you want to delete this song?')) {
      this.updateCachedSong(id, { deleted: true }).then(() => {
  ...
  private async addSongToDatabase(videoInfo: YouTubeVideoInfo, youtubeUrl: string, isBatch: boolean = false): Promise<boolean> {
    ...
    newSong.id = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    this.writeCachedSongs([...this.readCachedSongs(), newSong]);
  ...
  async openVideoPlayerDialog(song: Song): Promise<void> {
    ...
    dialogRef.afterClosed().subscribe((unavailableSong: Song | null) => {
      if (unavailableSong && unavailableSong.videoId) {
        ...
        this.updateCachedSong(unavailableSong.id, {
